#ifndef _SARSA_AGENT_HPP_
#define _SARSA_AGENT_HPP_
...
template <typename Env>
class sarsa_agent : public agent<Env, sarsa_transition<Env>> {
protected:
...
public:
    sarsa_agent(size_t state_size, size_t action_size, float alpha, float gamma, float init)
    ...
    void update(const sarsa_transition<Env>& t) override {
        float next_q = t.done ? 0.0f : Q_table_[idx(t.s_next, t.a_next)];
