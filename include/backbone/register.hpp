...
std::shared_ptr<agent<Env>> create_agent(Env& env, algo_type a, policy_type p, float epsilon, float gamma = 0.9f, float alpha = 0.5f, float init = 0.0f) {
    std::unique_ptr<policy<Env>> t_p = nullptr;
    std::unique_ptr<policy<Env>> b_p = nullptr;
    ...
    if (a == algo_type::TD) {
        if (p == policy_type::ON_POLICY) {
            ...
            b_p = std::make_unique<epsilon_greedy_policy<Env>>(env, epsilon);
            buffer = std::make_unique<nstep_buffer<Env>>(1);
    ...
    return std::make_shared<agent<Env>>(env, std::move(t_p), std::move(b_p), std::move(buffer), std::move(updater), gamma, alpha, init);
}
